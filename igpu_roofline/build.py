...
def build_host(jobs: int = 8, vulkan_include: str | None = None):
    ...
    cmd = ["cmake", "-S", str(paths.REPO / "runner"), "-B", str(paths.HOST_BUILD), "-DCMAKE_BUILD_TYPE=Release"]
    if vulkan_include:
        cmd.append(f"-DVULKAN_INCLUDE={Path(vulkan_include).expanduser().resolve()}")
    subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL)
    subprocess.run(["cmake", "--build", str(paths.HOST_BUILD), "-j", str(jobs)], check=True)
...
def build(jobs: int = 8, host: bool = False, shaders_too: bool = True, vulkan_include: str | None = None):
    ...
    if shaders_too:
        ...
        raise SystemExit("--no-shaders needs an existing build/shaders + build/shader-manifest.json (copy them from a build host).")
    if host:
