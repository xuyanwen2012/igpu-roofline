...
COMPONENT = {0: "f16", 1: "f32", 2: "f64", 3: "s8", 4: "s16", 5: "s32", 6: "s64", 7: "u8", 8: "u16",
             9: "u32", 10: "u64", 1000141000: "bf16", 1000491002: "e4m3", 1000491003: "e5m2"}
...
def table(caps: dict) -> str:
    ...
    rows = ["| M×N×K | A | B | C | Result | scope | saturating |", "|---|---|---|---|---|---|---|"]
    for x in caps.get("matrix_shapes", []):
        rows.append(f"| {x['m']}×{x['n']}×{x['k']} | {t(x['a'])} | {t(x['b'])} | {t(x['c'])} | {t(x['result'])} "
