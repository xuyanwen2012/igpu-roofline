...
def catalogue() -> list[tuple[str, str, dict, dict]]:
    ...
    def add(name, src, defines, meta):
        jobs.append(
            (
                ...
                src,
                defines,
                dict(meta, name=name, shader="shaders/" + name + ".spv"),
            )
        )
    ...
    for w in (1, 2, 4):
        ...
        for op, label in enumerate(
            ["read", "write", "copy", "scale", "add", "triad", "dot"]
        ):
            add(
                f"mem_{label}_v{w}",
                "memory",
                {"WIDTH": w, "OP": op},
                {"family": "memory", "op": op, "width": w, "dtype": "fp32"},
            )
        if w == 4:
            for op, label in enumerate(
                ["read", "write", "copy", "scale", "add", "triad"]
            ):
                add(
                    f"mem_{label}_v{w}_volatile",
                    "memory",
                    {"WIDTH": w, "OP": op, "VOLATILE": "volatile"},
                    {
                        "family": "memory",
                        "op": op,
                        "width": w,
        ...
        for dtype in ("fp32", "fp16"):
            ...
            for chains in (1, 2, 4, 8, 16):
                add(
                    f"alu_{dtype}_v{w}_c{chains}",
                    "alu",
                    {"WIDTH": w, "T": vec, "S": scalar, "CHAINS": chains},
                    {"family": "alu", "width": w, "chains": chains, "dtype": dtype},
                )
            # Workgroup memory, textbook form: 8 independent accumulators (read) / pure writes.
            for op in (0, 2):
                add(
                    f"sharedbw_{dtype}_v{w}_op{op}",
                    "shared_bw",
                    {"WIDTH": w, "T": vec, "S": scalar, "OP": op, "ACC": 8},
                    {
                        "family": "shared",
                        "kind": "bw",
                        "width": w,
            ...
            for acc in (4, 16, 32):
                add(
                    f"sharedbw_{dtype}_v{w}_op0_acc{acc}",
                    "shared_bw",
                    {"WIDTH": w, "T": vec, "S": scalar, "OP": 0, "ACC": acc},
                    {
                        "family": "shared",
                        "kind": "bw",
                        "width": w,
            ...
            for op in (0, 1):
                add(
                    f"shared_{dtype}_v{w}_op{op}_xlanes_volatile",
                    "shared_crosslane",
                    {"WIDTH": w, "T": vec, "S": scalar, "OP": op},
                    {
                        "family": "shared",
                        "width": w,
                        "op": op,
    ...
    add("pchase", "pchase", {}, {"family": "latency", "width": 1, "dtype": "uint32"})
    ...
    for fmt, texel in (("rgba16f", "f16vec4"), ("rgba32f", "vec4")):
        for mode, dim in (("buffer", 0), ("tex2d", 2), ("tex3d", 3)):
            add(
                f"texture_{fmt}_{mode}",
                "texture",
                {"TEX": dim, "TEXEL": texel},
                {
                    "family": "texture",
                    "width": 4,
                    "dtype": "fp16" if fmt == "rgba16f" else "fp32",
                    "format": fmt,
                    "tex_dim": dim,
                    "mode": mode,
                },
    ...
    for flops in ert_flops:
        add(
            ...
            "ert",
            {"FLOPS": flops},
            {"family": "ert", "width": 4, "dtype": "fp32", "flops_per_element": flops},
        )
    ...
    for elems in (8, 16):
        for flops in ert_flops:
            add(
                f"ert_f{flops}_e{elems}",
                "ert",
                {"FLOPS": flops, "ELEMS": elems},
                {
                    "family": "ert",
                    "width": 4,
                    "dtype": "fp32",
    ...
    shapes = (
        [(64, n, 16, "fp16") for n in (16, 32, 64)]
        + [(64, n, 32, "int8") for n in (16, 32, 64)]
        + [
            (8, 16, 16, "fp16"),
            (8, 16, 16, "fp16_fp32"),
            (8, 16, 32, "int8"),
    ...
    for m, n, k, dtype in shapes:
        ...
        for chains in (1, 2, 4, 8):
            add(
                f"matrix_{dtype}_{m}x{n}x{k}_c{chains}",
                "matrix",
                {"M": m, "N": n, "K": k, "AT": a_type, "CT": c_type, "CHAINS": chains},
                {
                    "family": "matrix",
                    "dtype": dtype,
                    "m": m,
            ...
            add(
                f"matrix_{dtype}_{m}x{n}x{k}_c{chains}_lds",
                "matrix",
                {
                    "M": m,
                    "N": n,
            ...
            add(
                f"matrix_{dtype}_{m}x{n}x{k}_c{chains}_gmem",
                "matrix",
                {
                    "M": m,
                    "N": n,
...
def _matrix_source(text: str, chains: int, dtype: str) -> str:
    ...
    text = text.replace(
        f"[[unroll]]for(uint c=0;c<CHAINS;c++)acc[c]={acc}(CT(c));",
        "\n".join(
            f"acc{c}={acc}(CT({c if dtype == 'int8' else c / 16}));"
    ...
    text = text.replace(
        ...
        "\n".join(
            f"coopMatStore(acc{c},d,((gl_WorkGroupID.x*gl_NumSubgroups+gl_SubgroupID)*CHAINS+{c})*M*N,N,gl_CooperativeMatrixLayoutRowMajor);"
            for c in range(chains)
        ),
    )
    return text
...
def _build_one(job) -> dict:
    ...
    if src == "matrix":
        ...
        expanded.write_text(
            _matrix_source(source.read_text(), defines["CHAINS"], meta["dtype"])
        )
        source = expanded
    spv = out / f"{name}.spv"
    cmd = (
        ["glslc", f"--target-env={GLSLC_TARGET}", "-O"]
        + [f"-D{k}={v}" for k, v in defines.items()]
        + [str(source), "-o", str(spv)]
    )
    p = subprocess.run(cmd, capture_output=True, text=True)
    if p.returncode:
        raise RuntimeError(f"{name}: glslc failed\n{p.stderr}")
    subprocess.run(
        ["spirv-val", "--target-env", GLSLC_TARGET, str(spv)],
        check=True,
        capture_output=True,
    )
    ...
    if bad:
        raise RuntimeError(
            f"{name}: static SPIR-V ledger mismatch (expected, actual): {bad}"
        )
