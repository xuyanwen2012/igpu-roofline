...
def manifest_runner(manifest: dict) -> str | None:
    ...
    return (
        ...
        or manifest.get("build/android/roofline")
...
def use_target(target: str) -> None:
    ...
    RUNNER, RUNNER_SUSTAINED, INSPECT = (
        build / "roofline",
        build / "roofline_sustained",
        build / "inspect",
    )
...
def results_root(override: str | None = None) -> pathlib.Path:
    path = (
        override or os.environ.get("IGPU_ROOFLINE_RESULTS") or "~/igpu-roofline-results"
    )
