...
def isa_summary(isa: str, m: dict) -> dict:
    ...
    return {
        "fma_scalar_equivalent": fma,
        "expected_scalar_fma": exp,
        "fma_matches_design": None if exp is None else fma == exp,
        ...
        "scratch": sum(1 for o in ops if o.startswith("scratch_")),
        "header": header,
    }
...
def _malioc(tool: str, core: str, spv: Path) -> dict:
    ...
    return {
        ...
        "properties": props,
        "total_cycles": dict(
            zip(perf["pipelines"], perf["total_cycles"]["cycle_count"])
        ),
...
def _rga(tool: str, asic: str, spv: Path, m: dict, out: Path) -> dict:
    with tempfile.TemporaryDirectory() as d:
        ...
        isa_files, stat_files = (
            list(Path(d).glob("*isa*")),
            list(Path(d).glob("*stats*")),
        )
    ...
    check = (
        ""
        if summary["expected_scalar_fma"] is None
        else (
            " (matches design)"
            if summary["fma_matches_design"]
    ...
    return {
        ...
        "tool": "rga",
        "asic": asic,
        "approximation": "AMD RDNA compiler, not the device driver's",
        "stats": st,
        "counts": summary,
        "spills": spill > 0,
...
def run(session) -> None:
    ...
    tool = (
        _tool("MALIOC" if tool_name == "malioc" else "RGA", tool_name)
        if tool_name
        else None
    )
    if not tool:
        reason = (
            "no public offline compiler for this GPU"
            if not tool_name
            else f"{tool_name} not found (set ${tool_name.upper()} or PATH)"
        )
        (out / "unavailable.json").write_text(
            json.dumps({"gpu": session.caps["gpu"], "reason": reason})
        )
    ...
    for m in session.manifest:
        ...
        if (
            getattr(session, "inspection_names", None) is not None
            and m["name"] not in session.inspection_names
        ):
            continue
        ...
        res = (
            _malioc(tool, str(tgt), spv)
            if tool_name == "malioc"
            else _rga(tool, str(tgt), spv, m, out)
        )
        res["shader"] = m["name"]
