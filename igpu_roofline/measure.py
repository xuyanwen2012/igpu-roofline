...
def stats(xs: list[float]) -> dict:
    ...
    return {
        ...
        "mean_seconds": mean,
        "stddev_seconds": sd,
        "cv": sd / mean if len(xs) > 1 else 0.0,
...
def differential(samples: list[dict], halves: dict) -> dict:
    ...
    """
    pairs = [
        (s["seconds"], halves[s["sample"]]["seconds"])
        for s in samples
        if s["sample"] in halves
    ]
    ...
    return {
        ...
        "half_loops": half,
        "pairs": len(pairs),
        "seconds_per_loop": per_loop,
        "fixed_seconds": fixed,
        "fixed_fraction": fixed / full,
...
def accounting(c: dict, loops: int) -> dict:
    ...
    a = {
        "float_ops": 0,
        "integer_ops": 0,
        "logical_global_bytes": 0,
        "logical_shared_bytes": 0,
        "physical_dram_bytes": None,
        "physical_cache_bytes": None,
    }
    ...
    if family == "memory":
        ...
        op = c["op"]
        a["logical_shared_bytes"] = (
            threads * width * scalar * loops * per * (1 if op in (0, 2) else 2)
        )
        a["logical_global_bytes"] = (
            c["groups"] * c["shared_count"] * width * 4 + threads * width * 4
        )
        a["shared_initialization_bytes"] = (
            c["groups"] * c["shared_count"] * width * scalar
        )
        ...
        subgroup = c.get("subgroup", c["wg"])
        if subgroup <= 0 or c["wg"] <= 0 or c["wg"] % subgroup:
            raise ValueError("matrix workgroup must contain whole subgroups")
        subgroups = c["wg"] // subgroup
        ops = (
            2
            * c["m"]
            * c["matrix_n"]
        ...
        a["logical_global_bytes"] = (
            c["groups"]
            * subgroups
            * (tile_bytes + c["m"] * c["matrix_n"] * chains * acc_bytes)
        )
        loads = (
            c["groups"] * subgroups * loops * tile_bytes
        )  # one A+B pair per iteration per subgroup
