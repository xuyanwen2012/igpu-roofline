...
def write_all(folder: Path, report: Path, all_rows: list, caps: dict, summary: dict):
    ok = [r for r in all_rows if r.get("accepted") and "accounting" in r]
...
def write_supplement(report: Path, ok: list, caps: dict):
    ...
    capacity = sorted(_stage(ok, "latency-capacity"), key=lambda r: r["accounting"]["working_set_bytes"])
    tlb = sorted(_stage(ok, "latency-tlb"), key=lambda r: r["accounting"]["working_set_bytes"])
    stride = sorted(_stage(ok, "latency-stride"), key=lambda r: (r["accounting"]["working_set_bytes"], r["accounting"]["chain_stride_bytes"]))
    L += ["## Load-to-use latency (pointer chasing)", "",
    ...
    L += [f"| {r['accounting']['working_set_bytes'] / 1024:g} KiB | {latency_ns(r):.1f} |" for r in capacity]
    if tlb:
        page = tlb[0]["config"].get("page_size")
        L += ["", f"| TLB test: pages touched (one node per {page} B page) | span | latency ns |", "|---:|---:|---:|"]
        L += [f"| {r['accounting']['working_set_bytes'] // page} | {r['accounting']['working_set_bytes'] / MiB:g} MiB | {latency_ns(r):.1f} |" for r in tlb]
    if stride:
        L += ["", "Line size (Saavedra): sequential chains over an array larger than the level under test. Below the "
              "line size several loads share a line; at the line size every load is a new line.", "",
              "| array | stride | avg latency ns |", "|---:|---:|---:|"]
        L += [f"| {r['accounting']['working_set_bytes'] / MiB:g} MiB | {r['accounting']['chain_stride_bytes']} B | {latency_ns(r):.1f} |" for r in stride]
    if capacity or stride:
        fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
        if capacity:
            axes[0].plot([r["accounting"]["working_set_bytes"] for r in capacity], [latency_ns(r) for r in capacity], ".-", label="64 B random")
        if tlb:
            axes[0].plot([r["accounting"]["working_set_bytes"] for r in tlb], [latency_ns(r) for r in tlb], "x--", label="1 per page (TLB)")
        axes[0].set(xscale="log", xlabel="working set (bytes)", ylabel="latency (ns)", title="capacity / TLB")
        axes[0].legend(fontsize=8)
        for size in sorted({r["accounting"]["working_set_bytes"] for r in stride}):
            pts = [(r["accounting"]["chain_stride_bytes"], latency_ns(r)) for r in stride if r["accounting"]["working_set_bytes"] == size]
            axes[1].plot([p[0] for p in pts], [p[1] for p in pts], ".-", label=f"{size / MiB:g} MiB array")
        axes[1].set(xscale="log", xlabel="stride (bytes)", ylabel="avg latency (ns)", title="line size")
        if stride:
    ...
    ert = _stage(ok, "ert")
    if ert:
        L += ["", "## ERT (Empirical Roofline Toolkit) sweep", "",
        ...
        for r in sorted(ert, key=lambda r: (r["config"]["flops_per_element"], r["config"]["wg"])):
            a, f = r["accounting"], r["config"]["flops_per_element"]
            L.append(f"| {f} | {f * 8 / 32:g} | {r['config']['wg']} | {a['float_ops'] / r['median_seconds'] / 1e9:.1f} | "
    ...
    if control:
        by = {}
        for r in control:
            by.setdefault(r["config"]["name"], {}).setdefault(r["config"].get("memory_mode", "device_local"), []).append(_rate(r))
        L += ["", "## Memory type (A/B)", "",
              "Same kernel and 256 MiB working set; only the buffer memory type changes. Arms alternate order each "
              "repeat; medians (range).", "", "| kernel | DEVICE_LOCAL GB/s | host-visible coherent GB/s | ratio |", "|---|---:|---:|---:|"]
        for name, v in sorted(by.items()):
            d, h = v.get("device_local"), v.get("host_coherent")
            if d and h:
                L.append(f"| {name} | {statistics.median(d):.1f} ({min(d):.1f}–{max(d):.1f}) | "
                         f"{statistics.median(h):.1f} ({min(h):.1f}–{max(h):.1f}) | {statistics.median(d) / statistics.median(h):.2f}× |")
        fallback = any(b.get("fallback_host_visible") for r in control for b in (r.get("allocation") or {}).get("buffers", []))
        if fallback:
...
def write_isa_check(folder: Path, report: Path, caps: dict, summary: dict):
    ...
    keep = ("fma", "dot", "coopmat_muladd", "load_StorageBuffer", "store_StorageBuffer", "load_Workgroup",
            "store_Workgroup", "control_barrier")
    L = [f"# {caps['gpu']} SPIR-V / driver / ISA check", "",
    ...
    for key, item in sorted(summary["short_run"].items()):
        ...
        m = manifest.get(name, {})
        ledger = ", ".join(f"{k}={v}" for k, v in m.get("spirv_ledger", {}).items() if k in keep and v)
        stats = _driver_stats(folder, name)
        driver = "not inspected" if stats is None else "; ".join(
            f"{k}={v}" for k, v in stats.items()
            if any(t in k.lower() for t in ("register", "spill", "instruction count all", "alu instruction", "arithmetic fma", "vgpr"))) or "none reported"
        isa_files = sorted((folder / "pipeline-inspection").glob(f"{name}.*ISA*.txt"))
        if isa_files and m:
            s = isa_summary(isa_files[0].read_text(), m)
            check = "" if s["expected_scalar_fma"] is None else (" ✓" if s["fma_matches_design"] else f" ✗ (design {s['expected_scalar_fma']})")
            isa = (f"driver ISA: VGPR {s['header'].get('vgpr_count', '?')}, wave{s['header'].get('wave_size', '?')}, "
            ...
            isa = json.loads(off.read_text()).get("summary", "see file") if off.exists() else "—"
...
def write_tuning(folder: Path, report: Path, ok: list, caps: dict):
    L = [f"# {caps['gpu']} shader tuning guide", "",
    ...
    for dtype in ("fp32", "fp16"):
        rows = [r for r in _stage(ok, "sweep-compute") if r["config"]["family"] == "alu" and r["config"]["dtype"] == dtype]
        if not rows:
            continue
        grid = {}
        for r in rows:
        ...
        L += [f"## {dtype} FMA: vector width × independent chains (GFLOP/s, best WG)", "",
              "| width \\ chains | 1 | 2 | 4 | 8 | 16 |", "|---|---:|---:|---:|---:|---:|"]
        for w in (1, 2, 4):
            cells = []
            for ch in (1, 2, 4, 8, 16):
                v = grid.get((w, ch))
                spill = " ⚠spill" if _offline(folder, f"alu_{dtype}_v{w}_c{ch}").get("spills") else ""
                cells.append(f"{v[0]:.0f}{spill}" if v else "—")
            L.append(f"| vec{w} | " + " | ".join(cells) + " |")
        need = {w: min((ch for ch in (1, 2, 4, 8, 16) if grid.get((w, ch), (0,))[0] >= 0.9 * top), default=None) for w in (1, 2, 4)}
        L += ["", f"- Fastest: vec{bw} × {bc} chains, WG {bwg}: {top:.0f} GFLOP/s.",
    ...
    regs = [(p.stem, json.loads(p.read_text())) for p in sorted((folder / "offline-isa").glob("alu_*.json"))] if (folder / "offline-isa").exists() else []
    regs = [(n, o) for n, o in regs if o.get("ok")]
    if regs:
        spill = sorted(n for n, o in regs if o.get("spills"))
        low = sorted(n for n, o in regs if (o.get("properties") or {}).get("thread_occupancy", 100) < 100)
        L += ["## Register budget (offline compiler)", "",
    ...
    dram = [r for r in _stage(ok, "sweep-cache") if r["config"]["op"] == 0 and r["accounting"].get("working_set_bytes", 0) >= 256 * MiB]
    ops = {}
    for r in _stage(ok, "sweep-memory"):
        c = r["config"]
        if not c.get("volatile_global") and r["accounting"].get("working_set_bytes", 0) >= 256 * MiB:
    ...
    if dram:
        ...
        L += ["## DRAM streaming read: vector width × WG (GB/s, >= 256 MiB)", "",
              "| width | WG 64 | WG 128 | WG 256 |", "|---|---:|---:|---:|"]
        L += [f"| vec{w} | " + " | ".join(f"{g[(w, wg)]:.1f}" if (w, wg) in g else "—" for wg in (64, 128, 256)) + " |" for w in (1, 2, 4)]
        L += ["", f"- Best: vec{bw} ({bw * 4} B per access), WG {bwg}: {bv:.1f} GB/s; scalar reads reach {v1 / bv:.0%} of it."]
    if ops:
        L.append("- DRAM bandwidth by kernel (GB/s): " + ", ".join(f"{k} {v:.1f}" for k, v in ops.items()) + "."
                 + (f" Writes reach only {ops['write'] / ops['read']:.0%} of reads: coalesce output into wide stores."
                    if "read" in ops and "write" in ops and ops["write"] < 0.85 * ops["read"] else ""))
    L.append("")

    # cache working set
    cache_rows = [r for r in _stage(ok, "sweep-cache") if r["config"]["op"] == 0 and r["config"]["width"] == 4]
    by_ws = {}
    for r in cache_rows:
    ...
    if by_ws:
        dram_bw = min((v for ws, v in by_ws.items() if ws >= 256 * MiB), default=min(by_ws.values()))
        fast = [ws for ws, v in sorted(by_ws.items()) if v >= 2 * dram_bw]
        if fast:
            L += ["## Cache working set", f"- Data re-read within a dispatch stays >= 2× DRAM bandwidth up to {max(fast) / MiB:g} MiB "
                  f"(best {cache_best:.0f} GB/s vs DRAM {dram_bw:.0f} GB/s). Size tiles so the reused set fits in this.", ""]

    # shared memory
    sh = [r for r in _stage(ok, "sweep-shared") if r["config"].get("kind") == "bw" and r["config"]["op"] == 0]
    if sh:
        best = max(sh, key=_rate)
        sv = _rate(best)
        L += ["## Shared (workgroup) memory",
              f"- Best shared read {sv:.0f} GB/s ({best['config']['name']}, WG {best['config']['wg']})"
              + (f"; cache-reuse read {cache_best:.0f} GB/s; ratio {sv / cache_best:.2f}×." if cache_best else ".")]
        if cache_best:
            L.append("- " + ("Shared memory is clearly faster than the cache: staging reused data in shared memory pays off."
                             if sv > 1.3 * cache_best else
                             "Shared memory is not faster than the cache here: rely on cache locality (bounded working sets, "
        ...
        if len(accs) > 1:
            peak_acc = max(accs, key=accs.get)
            L.append("- Independent accumulators vs shared read bandwidth: " + ", ".join(f"{k}: {v:.0f}" for k, v in sorted(accs.items()))
                     + " GB/s. " + ("More accumulators no longer help: shared reads are saturated."
                                    if accs[peak_acc] <= 1.1 * accs.get(8, 0) else
                                    f"Up to {peak_acc} independent reads in flight still help: issue enough independent shared loads per thread."))
        for dtype in ("fp32", "fp16"):
            scalar = 2 if dtype == "fp16" else 4
            st = {r["config"]["stride"]: _rate(r) for r in sh if r["config"]["dtype"] == dtype and r["config"]["width"] == 1
                  and r["config"]["wg"] == 64 and r["config"].get("accumulators") == 8
                  and r["config"]["shared_count"] * scalar == caps["max_shared_bytes"]}
            if 1 in st and len(st) > 2:
                L.append(f"- {dtype} scalar stride sensitivity (vs stride 1): " + ", ".join(f"s{k} {v / st[1]:.2f}" for k, v in sorted(st.items())) + ".")
                bad = [k for k, v in st.items() if v < 0.7 * st[1]]
                if bad:
                    recovered = any(st.get(k, 0) >= 0.9 * st[1] for k in (33, 65))
                    L.append(f"  - Strides {bad} are slow (bank conflicts): pad 2-D shared arrays by one element per row "
                             f"(32→33, 64→65)." + (" Measured 33/65 recover." if recovered else
                                                   " Odd strides do not recover either: not a simple modulo bank mapping; verify padding by measurement."))
                else:
                    L.append("  - No stride penalty: no bank conflicts to pad around.")
        L.append("")

    # latency / TLB / loads in flight
    cap_rows = sorted(_stage(ok, "latency-capacity"), key=lambda r: r["accounting"]["working_set_bytes"])
    if cap_rows:
        small, big = latency_ns(cap_rows[0]), latency_ns(cap_rows[-1])
        L += ["## Latency and loads in flight",
        ...
        if ops.get("read"):
            L.append(f"- Little's law: saturating DRAM needs ~{ops['read'] * big / 1e3:.0f} KB of reads in flight GPU-wide "
        ...
        if tlb:
            ...
            reach = [r["accounting"]["working_set_bytes"] for r in tlb if latency_ns(r) <= 1.3 * base]
            if reach:
                L.append(f"- TLB reach ~{max(reach) / MiB:g} MiB (largest span with one access per page within 1.3× of the "
    ...
    if pk:
        L += ["## Precision and data type", "- Peaks (G ops/s): " + ", ".join(f"{k} {v:.0f}" for k, v in sorted(pk.items())) + "."]
        if "alu_fp16" in pk and "alu_fp32" in pk:
            ratio = pk["alu_fp16"] / pk["alu_fp32"]
            L.append(f"- fp16 is {ratio:.2f}× fp32" + (": worth it for compute-bound kernels." if ratio > 1.5 else
                                                        ": little compute gain; the main benefit of fp16 is halving bytes."))
        if "dot_int8" in pk and "alu_fp16" in pk:
            L.append(f"- int8 dot4 (8 ops each) runs at {pk['dot_int8'] / pk['alu_fp16']:.2f}× fp16 FMA throughput.")
        L.append("")

    # memory type
    ctl = {}
    for r in _stage(ok, "control-memory-type"):
        ctl.setdefault(r["config"]["name"], {}).setdefault(r["config"].get("memory_mode", "device_local"), []).append(_rate(r))
    if ctl:
        L.append("## Buffer memory type")
        for name, v in sorted(ctl.items()):
            if "device_local" in v and "host_coherent" in v:
                d, h = statistics.median(v["device_local"]), statistics.median(v["host_coherent"])
                L.append(f"- {name}: DEVICE_LOCAL {d:.1f} GB/s vs host-visible coherent {h:.1f} GB/s ({d / h:.2f}×).")
        L.append("- Keep large GPU-only buffers in DEVICE_LOCAL memory; move data to/from the CPU through staging copies.")
