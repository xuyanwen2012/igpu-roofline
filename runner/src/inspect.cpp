...
int main(int argc, char **argv) {
  try {
    ...
    if (family == "matrix") {
      sizes[0] = M * K * (integer ? 1 : 2);
      sizes[1] = K * N * (integer ? 1 : 2);
      sizes[3] = groups * chains * M * N * (half ? 2 : 4);
    }
    Buf b[4];
    for (int i = 0; i < 4; i++) {
      if (sizes[i] > c.props.limits.maxStorageBufferRange)
    ...
    for (uint32_t i = 0; i < 4; i++)
      binds[i] = {i,
                  (family == "texture" && cfg.value("tex_dim", 0u) != 0 && i == 2)
    ...
    auto props = (PFN_vkGetPipelineExecutablePropertiesKHR)vkGetDeviceProcAddr(
        c.dev, "vkGetPipelineExecutablePropertiesKHR");
    auto stats = (PFN_vkGetPipelineExecutableStatisticsKHR)vkGetDeviceProcAddr(
        c.dev, "vkGetPipelineExecutableStatisticsKHR");
    auto reps = (PFN_vkGetPipelineExecutableInternalRepresentationsKHR)vkGetDeviceProcAddr(
        c.dev, "vkGetPipelineExecutableInternalRepresentationsKHR");
    VkPipelineInfoKHR info{VK_STRUCTURE_TYPE_PIPELINE_INFO_KHR};
    info.pipeline = pipe;
    uint32_t num = 0;
    CHECK(props(c.dev, &info, &num, nullptr));
    std::vector<VkPipelineExecutablePropertiesKHR> pp(num);
    for (auto &v : pp)
      v.sType = VK_STRUCTURE_TYPE_PIPELINE_EXECUTABLE_PROPERTIES_KHR;
    CHECK(props(c.dev, &info, &num, pp.data()));
    ...
    for (uint32_t i = 0; i < num; i++) {
      ...
      uint32_t ns = 0;
      CHECK(stats(c.dev, &ei, &ns, nullptr));
      std::vector<VkPipelineExecutableStatisticKHR> ss(ns);
      for (auto &v : ss)
        v.sType = VK_STRUCTURE_TYPE_PIPELINE_EXECUTABLE_STATISTIC_KHR;
      CHECK(stats(c.dev, &ei, &ns, ss.data()));
      json ex = {{"name", pp[i].name},
                 {"description", pp[i].description},
                 {"subgroup_size", pp[i].subgroupSize},
                 {"statistics", json::array()},
                 {"representations", json::array()}};
      for (auto &st : ss) {
        json value;
        switch (st.format) {
        case VK_PIPELINE_EXECUTABLE_STATISTIC_FORMAT_BOOL32_KHR:
          value = bool(st.value.b32);
          break;
        ...
        ex["statistics"].push_back(
            {{"name", st.name}, {"description", st.description}, {"value", value}});
      }
      uint32_t nr = 0;
      CHECK(reps(c.dev, &ei, &nr, nullptr));
      std::vector<VkPipelineExecutableInternalRepresentationKHR> rr(nr);
      for (auto &v : rr)
        v.sType = VK_STRUCTURE_TYPE_PIPELINE_EXECUTABLE_INTERNAL_REPRESENTATION_KHR;
      if (nr) {
        CHECK(reps(c.dev, &ei, &nr, rr.data()));
        std::vector<std::vector<char>> buffers(nr);
        for (uint32_t k = 0; k < nr; k++) {
          buffers[k].resize(rr[k].dataSize);
          rr[k].pData = buffers[k].data();
        }
        CHECK(reps(c.dev, &ei, &nr, rr.data()));
        for (uint32_t k = 0; k < nr; k++)
          ex["representations"].push_back(
              {{"name", rr[k].name},
               {"description", rr[k].description},
               {"text", rr[k].isText ? std::string(buffers[k].data(),
                                                   strnlen(buffers[k].data(), buffers[k].size()))
