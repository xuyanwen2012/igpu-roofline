...
static Ctx createCtx() {
    ...
    VkPhysicalDeviceVulkan13Features f13{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES, hasCm?(void*)&cm:nullptr};
    ...
    VkPhysicalDeviceVulkan13Features e13{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES, cm.cooperativeMatrix?(void*)&eCm:nullptr};
    ...
    std::vector<const char*> exts; if (cm.cooperativeMatrix) exts.push_back("VK_KHR_cooperative_matrix");
...
static void freeBuf(Ctx& c, Buf& b) { if (b.p) vkUnmapMemory(c.dev, b.m); vkDestroyBuffer(c.dev, b.b, nullptr); vkFreeMemory(c.dev, b.m, nullptr); }
...
struct DevBuf { VkBuffer b{}; VkDeviceMemory m{}; uint32_t type = 0; uint32_t flags = 0; bool fallback = false; };
static DevBuf makeDeviceBuf(Ctx& c, size_t size) {
    ...
    for (uint32_t i = 0; i < mp.memoryTypeCount && pick == UINT32_MAX; i++)
        if ((mr.memoryTypeBits & (1u << i)) && (mp.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) && !(mp.memoryTypes[i].propertyFlags & (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT | VK_MEMORY_PROPERTY_PROTECTED_BIT))) pick = i;
    if (pick == UINT32_MAX) {
        for (uint32_t i = 0; i < mp.memoryTypeCount && pick == UINT32_MAX; i++)
            if ((mr.memoryTypeBits & (1u << i)) && (mp.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) && !(mp.memoryTypes[i].propertyFlags & (VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT | VK_MEMORY_PROPERTY_PROTECTED_BIT))) pick = i;
        r.fallback = true;
    }
    if (pick == UINT32_MAX) { fprintf(stderr, "No device-local memory type for buffer\n"); exit(2); }
    r.type = pick; r.flags = mp.memoryTypes[pick].propertyFlags;
    VkMemoryAllocateInfo mai{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO}; mai.allocationSize = mr.size; mai.memoryTypeIndex = pick;
    CHECK(vkAllocateMemory(c.dev, &mai, nullptr, &r.m));
    CHECK(vkBindBufferMemory(c.dev, r.b, r.m, 0));
    fprintf(stderr, "DEVICE BUFFER bytes=%zu type=%u flags=%u fallback=%d\n", size, r.type, r.flags, (int)r.fallback);
    return r;
}
...
static void copyNow(Ctx& c, VkBuffer src, VkBuffer dst, size_t size) {
    ...
    VkCommandBuffer cb; CHECK(vkAllocateCommandBuffers(c.dev, &ca, &cb));
    VkCommandBufferBeginInfo bi{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO}; bi.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT; CHECK(vkBeginCommandBuffer(cb, &bi));
    VkMemoryBarrier mb{VK_STRUCTURE_TYPE_MEMORY_BARRIER}; mb.srcAccessMask = VK_ACCESS_HOST_WRITE_BIT | VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_TRANSFER_WRITE_BIT; mb.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_TRANSFER_WRITE_BIT;
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT | VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 1, &mb, 0, nullptr, 0, nullptr);
    VkBufferCopy region{0, 0, size}; vkCmdCopyBuffer(cb, src, dst, 1, &region);
    mb.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT; mb.dstAccessMask = VK_ACCESS_HOST_READ_BIT | VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT | VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 1, &mb, 0, nullptr, 0, nullptr);
    CHECK(vkEndCommandBuffer(cb));
    VkFenceCreateInfo fc{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO}; VkFence f; CHECK(vkCreateFence(c.dev, &fc, nullptr, &f));
    VkSubmitInfo si{VK_STRUCTURE_TYPE_SUBMIT_INFO}; si.commandBufferCount = 1; si.pCommandBuffers = &cb;
    CHECK(vkQueueSubmit(c.q, 1, &si, f)); CHECK(vkWaitForFences(c.dev, 1, &f, VK_TRUE, 60000000000ull));
