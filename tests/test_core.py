...
from igpu_roofline.stages import PLANS
from igpu_roofline.stages import is_control as config_is_control
from igpu_roofline.volatile_workgroup import annotate


# --- accounting ------------------------------------------------------------------------
def test_batch_multiplies_traffic_not_capacity():
    c = {"family": "memory", "op": 2, "n": 1024, "width": 4, "wg": 64, "groups": 16}
...
def test_babelstream_dot():
    a = accounting(
        {"family": "memory", "op": 6, "n": 1024, "width": 4, "wg": 64, "groups": 16}, 2
    )
...
def test_dot8_counts_every_dot():
    assert (
        accounting(
            {"family": "dot", "chains": 4, "wg": 64, "groups": 8, "dots_per_step": 8},
            10,
        )["integer_ops"]
        == 8 * 64 * 8 * 4 * 10 * 8
    )
...
def test_ert_intensity():
    a = accounting(
        {"family": "ert", "flops_per_element": 64, "n": 1000, "wg": 64, "groups": 4}, 3
    )
...
def test_differential_removes_fixed_cost():
    ...
    samples = [
        {"sample": i, "loops": L, "seconds": fixed + L * per_loop + (i % 3) * 1e-6}
        for i in range(9)
    ]
    halves = {
        i: {"sample": i, "loops": H, "seconds": fixed + H * per_loop + (i % 3) * 1e-6}
        for i in range(9)
    }
...
def test_differential_rejects_negative_overhead():
    d = differential(
        [{"sample": 0, "loops": 100, "seconds": 0.010}],
        {0: {"sample": 0, "loops": 50, "seconds": 0.004}},
    )
    assert not d["valid"]
...
def test_timestamp_wrap_arithmetic():
    ...
    assert ((3 - ((1 << 48) - 2)) & ((1 << 48) - 1)) * period == pytest.approx(
        260.4166603088379
    )
...
def test_ledger_mismatch_is_reported():
    assert check({"family": "alu", "chains": 1}, {"fma": 15}) == {"fma": (16, 15)}


def test_volatile_only_on_workgroup_accesses():
    src = (
        "%pw = OpTypePointer Workgroup %float\n%pf = OpTypePointer Function %float\n%w = OpAccessChain %pw %data %i\n"
        "%f = OpVariable %pf Function\n%a = OpLoad %float %w\n%b = OpLoad %float %f\nOpStore %w %a\nOpStore %f %b\n"
    )
    out, n = annotate(src)
    assert n == 2
    assert (
        "OpLoad %float %w Volatile" in out
        and "OpLoad %float %f\n" in out
        and "OpStore %w %a Volatile" in out
    )
...
def test_amd_isa_fma_counting():
    ...
    assert expected_scalar_fma({"family": "alu", "chains": 16, "width": 4}) == 1024
...
def test_controls_never_define_roofs():
    ...
    keys = [
        metric(dict(base, config=cfg))[0]
        for cfg in (
            {"family": "memory", "op": 2, "volatile_global": True},
            {"family": "memory", "op": 1, "memory_mode": "host_coherent"},
            {"family": "shared", "op": 0, "dtype": "fp32"},
            {"family": "shared", "kind": "bw", "op": 0, "dtype": "fp32"},
        )
    ]
    assert keys == [
        "global_copy_volatile",
        "global_write_hostcoherent",
        "shared_fp32_read_1acc",
        "shared_fp32_read",
    ]
    assert [is_control(k) for k in keys] == [True, True, True, False]
    assert config_is_control({"family": "shared", "op": 0}) and not config_is_control(
        {"family": "shared", "kind": "bw", "op": 0}
    )
...
def test_partial_sustained_keeps_short_roofs():
    short = {
        "alu_fp32": {"value": 4, "unit": "TFLOP/s", "confirmed": True},
        "global_copy": {"value": 80, "unit": "GB/s", "confirmed": True},
    }
    assert choose_roofs(
        short, {"alu_fp32": {"value": 3, "unit": "TFLOP/s", "batches": 1}}
    ) == (short, "short-run")
    full = {k: dict(v, batches=3, all_steady=True) for k, v in short.items()}
...
def test_plans():
    ...
    assert PLANS["fast"]["sustain"]["duration"] == 120 and PLANS["fast"]["confirm"] == {
        "top": 2,
        "reps": 3,
    }
...
from igpu_roofline.spirv_audit import expected
from igpu_roofline.stages import matrix_grid, quality
...
def _row(**kw):
    r = {
        ...
        "cv": 0.01,
        "n": 21,
        "below_target_duration": False,
        "differential": {"valid": True, "fixed_fraction": 0.02},
    }
    r.update(kw)
    return r
...
def test_quality_gates():
    ...
    assert "fixed_cost" in quality(
        _row(differential={"valid": True, "fixed_fraction": 0.67})
    )
    assert "fixed_cost" in quality(
        _row(differential={"valid": False, "fixed_fraction": 0.0})
    )
    assert "rejected" in quality(_row(accepted=False))
    assert (
        quality(_row(differential=None)) == []
    )  # streaming kernels have no differential


def test_matrix_grid_reaches_large_launches_for_every_dtype():
    caps = {"subgroup": 16, "max_workgroup_invocations": 1024}
    small = {"chains": 8, "m": 4, "matrix_n": 16, "dtype": "int8"}
    assert (16, 16384) in matrix_grid(small, caps, quick=True)
    assert {(16, 16384), (64, 16384), (64, 64)} <= set(
        matrix_grid(small, caps, quick=False)
    )
    big = {"chains": 8, "m": 64, "matrix_n": 64, "dtype": "fp16_fp32"}
    assert all(
        g * 8 * 64 * 64 * 4 <= 256 * 1024**2
        for _, g in matrix_grid(big, caps, quick=False)
    )


def test_ert_ledger_chunking_and_ilp():
    assert expected({"family": "ert", "flops_per_element": 16})["fma"] == 64
    assert expected({"family": "ert", "flops_per_element": 1024})["fma"] == 4 * 32
    assert (
        expected({"family": "ert", "flops_per_element": 1024, "elems": 16})["fma"]
        == 16 * 32
    )
    assert (
        expected({"family": "ert", "flops_per_element": 1024, "full_unroll": True})[
            "fma"
        ]
        == 4096
    )
    assert (
        expected({"family": "ert", "flops_per_element": 8, "elems": 8})[
            "load_StorageBuffer"
        ]
        == 8
    )
...
def test_plans_confirm_roofs():
    assert PLANS["quick"]["confirm"] == {"top": 1, "reps": 3}
    assert all(
        PLANS[p]["confirm"] == {"top": 3, "reps": 5} for p in ("standard", "gold")
    )
...
from igpu_roofline.stages import Guard
...
def test_gpu_temperature_reads_current_hal_section(monkeypatch):
...
def test_quarantine_moves_only_results_after_last_good_sentinel(tmp_path):
    def result(stage, key, utc):
        ...
        (d / f"{key}.telemetry.json").write_text(_json.dumps([{"utc": utc}]))
    ...
    assert (
        len(list((tmp_path / "superseded").glob("degraded-*/sweep-compute/late.*")))
        == 3
    )
...
def test_guard_needs_confirmed_drop_against_median(monkeypatch):
    ...
    g = Guard.__new__(Guard)
    g.s, g.plan, g.busy, g.readings, g.last_good_utc = (
        None,
        None,
        False,
        [3.3, 3.4, 3.58],
...
def test_drifting_or_unsteady_rows_are_gated():
    assert "drifting" in quality(
        _row(sample_drift=2.4)
    )  # 780M shared read: 15 -> 4.4 ms
    assert "warmup_unsteady" in quality(_row(warmup={"steady": False}))
    assert quality(_row(sample_drift=0.02, warmup={"steady": True})) == []
...
def test_only_current_runner_and_shader_rows_count():
    ...
    assert s.current(
        {"config": {"name": "matrix_x", "runner_sha256": "R2", "spirv_sha256": "S2"}}
    )
    assert not s.current(
        {"config": {"name": "matrix_x", "runner_sha256": "R1", "spirv_sha256": "S2"}}
    )  # old runner
    assert not s.current(
        {"config": {"name": "matrix_x", "runner_sha256": "R2", "spirv_sha256": "S1"}}
    )  # old shader
...
def test_matrix_feed_accounting_and_keys():
    ...
    assert (
        plain["float_ops"] == 2 * 16 * 16 * 16 * 4 * 100 * 2 * 10
    )  # 2 subgroups per workgroup
    tile = (16 * 16 + 16 * 16) * 2
    ...
    assert (
        matrix_feed_key(dict(base, feed="global"), gm) == "matrix_fp16_fp32_feed_dram"
    )
    assert (
        matrix_feed_key(dict(base, feed="global"), {"working_set_bytes": 1 << 20})
        == "matrix_fp16_fp32_feed_cache"
    )
    assert (
        matrix_feed_key(dict(base, feed="shared"), sh) == "matrix_fp16_fp32_feed_shared"
    )
...
def test_dram_fed_matrix_roof_is_a_bandwidth():
    ...
    gm = {
        "config": dict(base, feed="global"),
        "accounting": accounting(dict(base, feed="global", n=1 << 20), 10),
        "median_seconds": 0.5,
    }
    ...
    sh: dict = dict(
        gm,
        config=dict(base, feed="shared"),
        accounting=accounting(dict(base, feed="shared", tiles=4, n=4), 10),
    )
    assert (
        metric(sh)[2] == "TOP/s" and rate(sh) == sh["accounting"]["integer_ops"] / 0.5
    )
...
def test_texture_accounting_and_keys():
    ...
    assert expected({"family": "texture", "tex_dim": 3}) == {
        "image_fetch": 1,
        "store_StorageBuffer": 1,
    }
